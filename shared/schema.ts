...
// Status flow: sent (server has it) -> delivered (receiver socket got it) -> read (receiver opened chat)
// clientId enables idempotent sends and exact-match reconciliation between optimistic UI and server ack.
export const chatMessages = pgTable("chat_messages", {
  ...
  message: text("message").notNull(),
  // Idempotency / dedup key generated by the client.
  ...
  // Reactions stored as { "<emoji>": ["<userId>", ...] } so multiple users can react and we know who.
  ...
  // Unread-count queries hit (receiverId, read).
