    ...
    <Sheet open={mobileNotificationsOpen} onOpenChange={setMobileNotificationsOpen}>
      <SheetTrigger asChild>
        <Button variant="ghost" size="icon" className="relative" data-testid="button-notifications-mobile">
          <Bell className="h-5 w-5" />
          {unreadCount > 0 && (
            <span className="absolute -top-1 -right-1 h-5 w-5 rounded-full bg-primary text-[10px] font-medium text-primary-foreground flex items-center justify-center" data-testid="notification-badge-mobile">
              {unreadCount > 9 ? "9+" : unreadCount}
            </span>
          )}
        </Button>
