...
export default function PostcodeSearch({
  ...
  return (
    <div className={compact ? "space-y-1" : "space-y-2"} ref={containerRef}>
      {label && <label className={`${compact ? 'text-xs' : 'text-sm'} font-medium ${labelClassName}`}>{label}</label>}
      
      <div className="relative">
        {showPulse ? (
          <div className={`absolute left-2 ${compact ? 'top-1.5' : 'top-3'} z-10`}>
            <div className={`relative ${compact ? 'h-4 w-4' : 'h-6 w-6'} flex items-center justify-center`}>
              <div className={`absolute ${compact ? 'h-4 w-4' : 'h-6 w-6'} rounded-full bg-sky-400/30 animate-ripple-slow`} />
              <div className={`absolute ${compact ? 'h-3.5 w-3.5' : 'h-5 w-5'} rounded-full bg-sky-400/50 animate-ripple-slow animation-delay-300`} />
              <div className={`${compact ? 'h-2 w-2' : 'h-2.5 w-2.5'} rounded-full bg-sky-500 border-2 border-white shadow-sm`} />
            </div>
          </div>
        ) : (
          <MapPin className={`absolute ${compact ? 'left-2 top-2 h-3.5 w-3.5' : 'left-3 top-3 h-4 w-4'} ${iconColor} z-10`} />
        )}
        <Input
        ...
        />
        {isSearching && (
          <Loader2 className={`absolute ${compact ? 'right-2 top-2 h-3.5 w-3.5' : 'right-3 top-3 h-4 w-4'} animate-spin text-muted-foreground`} />
        )}
